...
/**
 * A simple uniform mode for a sequence with a given alphabet size. The
 * characters of the sequence are not checked for validitity
 */
...
class Markov0_DNA implements Seq_Model {
    ...
    Markov0_DNA(int alphaSize) {
        Misc.my_assert(alphaSize == this.alphaSize,
                "Markov0_DNA class only works for an alphabet of "
                        + this.alphaSize);
...
class Model_SeqA_DNA implements Two_Seq_Model_Counts {
    ...
    public Model_SeqA_DNA(Params p, int alphaSize, int countIndex) {
        ...
        Misc.my_assert(Model_SeqA_DNA.alphaSize == alphaSize,
                "Model_SeqA_DNA only works for an alphabet of "
    ...
    public Params counts_to_params(Counts counts) {
        ...
        for (int j = 0; j < alphaSize; j++) {
            ...
            for (int i = 0; i < alphaSize; i++)
                par.put("match_" + chars[i] + "|" + chars[j], -MyMath
                        .log2(counts.counts[countIndex
                                + pair_to_int(chars[i], chars[j])]
                                / sum));
        }
...
public class FuzzyLZ implements Seq_Model {
    ...
    static abstract class Matches implements Serializable {
        ...
        public Params counts_to_params(Counts c) {
            ...
            p.put("cont_cost", -MyMath
                    .log2(c.get(countIndex + contIndex) / sum));
            return p;
        }
    }
        ...
        for (int i = 0; i < M_class.length; i++) {
            try {
                ...
                M_new[i] = M_class[i].getConstructor(new Class[] {
                        Class.forName("common.Two_Seq_Model_Counts"),
                        Class.forName("common.Params"), Integer.TYPE,
                        Integer.TYPE });
            } catch (Exception e) {
                System.err.println("Error finding class/method for class '"
    ...
    public Params counts_to_params() {
        ...
        for (int m = 0; m < numFwd + numRev; m++) {
            res.put("start" + m + "_cost", -MyMath.log2(counts.get(noStartIndex
                    + 1 + m)
                    / sum));
        }
