...
/*
 * This class implements a sparse array to keep track of the
 * active cells in the DPA
 */
...
class Sparse implements Serializable {
    ...
    /** Class Iterate is used by a number of functions to
	store state information about the current position */
    ...
    */
	...
	if (prev == null) {
	    ...
	    if (prev.next != null) prev.next.prev = ins;
	    ins.next = prev.next;
	    ins.prev = prev;
	    prev.next = ins;
	}
    ...
    // Join any touching or overlapping sparse elements.  Assumes no useful data in the array
    void join() {
	Link l=head, p=null;
	...
	// Now set its size to 0, and realloc() so all Object are reclaimed in our temp list
	l.end=l.start;
	l.realloc();
    }

    // Add a new one from 'n1 to n2'
    // NOTE: This function assumes that no useful object data is currently in the array.
    //       This function does _not_ allocate room in links.
    //       After using this function, checkAlloc() must be called before using this sparse array
	...
	while (l != null) {
	    ...
	    if (s >= len) {	// No elements found < cutoff.  Throw away sparse list
		Link n = l.next;
		remove(l);
		l = n;
		continue;
    ...
    // moveFwd(int, Iterate) - find the element at 'int'.  iterate.o==null if none there.
