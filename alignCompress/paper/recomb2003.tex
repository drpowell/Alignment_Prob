...
\begin{figure}
\centering
\epsfig{file=3state.eps, width=0.4\columnwidth}
\caption{\label{fig:3state}DPA for affine gap costs aka. 3-State mutation model.}
\end{figure}

\begin{figure}
\centering
\epsfig{file=local.eps, width=0.3\columnwidth}
...
score and the M-alignment algorithm perform similarly.  This test is
relatively uninteresting and is only presented to show that for high entropy
sequences the M-alignment algorithm performs like existing algorithms.  Recall
that the M-alignment algorithm may take as a parameter the model for the
sequences.  Except where noted otherwise, the M-alignment algorithm is told to
fit a first order Markov Model to the sequences.
...
well as new M-alignment method when there are different models in the
population.  Similar behaviour has been seen when using a other combinations
of models.
