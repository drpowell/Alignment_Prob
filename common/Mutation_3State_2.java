...
public abstract class Mutation_3State_2 extends Mutation_FSM {
	...
	*/
	...
	void normalize_costs() {
	    ...
	    sum = MyMath.exp2(-diag) + 2*MyMath.exp2(-start);
	    diag_fromD  = diag  + MyMath.log2( sum );
	    start_fromD = start + MyMath.log2( sum );

	    sum = MyMath.exp2(-diag) + MyMath.exp2(-start) + MyMath.exp2(-cont);
	    diag_fromI  = diag  + MyMath.log2( sum );
	    start_fromI = start + MyMath.log2( sum );
	    cont_fromI  = cont  + MyMath.log2( sum );
	}
    ...
    final protected int diag=0, start=1, cont=2;

    public Mutation_3State_2(Two_Seq_Model_Counts s, Params par, int numCounts, int countIndex) {
	super(numCounts, countIndex);
	p = new FSM_Params(s, par);
	reset();
    }
    
    public Mutation_3State_2(FSM_Params p, int numCounts, int countIndex) { 
	super(numCounts, countIndex);
	this.p = p; 
	reset();
    }
	...
	double sum = c.counts[countIndex+diag] + c.counts[countIndex+start] +
	    c.counts[countIndex+cont];

	par.put("diag", -MyMath.log2(c.counts[countIndex+diag]/sum));
	par.put("start", -MyMath.log2(c.counts[countIndex+start]/sum));
	par.put("cont", -MyMath.log2(c.counts[countIndex+cont]/sum));
	...
	double len = p.s.encode_params(c.counts[countIndex+diag]);
	...
	dataLen = c.counts[countIndex+diag] + c.counts[countIndex+start] + c.counts[countIndex+cont];
	...
	Counts c = get_counts();
	return c.counts[countIndex+diag] + 
	       c.counts[countIndex+start] + 
	       c.counts[countIndex+cont];
    }
    ...
    */
    ...
    public static class One extends Mutation_3State_2 implements TraceBack_Info {
	...
	public Object clone() {
	    return new Mutation_3State_2.One(p, numCounts, countIndex);
	}
	...
	public void calc(Mutation_FSM h, Mutation_FSM v, Mutation_FSM d, 
	    ...
	    Mutation_3State_2.One hcell = (Mutation_3State_2.One)h;
	    Mutation_3State_2.One vcell = (Mutation_3State_2.One)v;
	    Mutation_3State_2.One dcell = (Mutation_3State_2.One)d;
		...
		tcounts.inc(countIndex+start, 1);
	...
	*/
    ...
    public static class All extends Mutation_3State_2 {
	...
	public Object clone() {
	    return new Mutation_3State_2.All(p, numCounts, countIndex);
	}
	...
	public void calc(Mutation_FSM h, Mutation_FSM v, Mutation_FSM d, 
			 char a, char b, int i, int j) {
	    Mutation_3State_2.All hcell = (Mutation_3State_2.All)h;
	    Mutation_3State_2.All vcell = (Mutation_3State_2.All)v;
	    Mutation_3State_2.All dcell = (Mutation_3State_2.All)d;
		...
		hcell.h_counts.inc(countIndex+start, count_inc);
		...
		hcell.p.s.update_count_encB(hcell.h_counts, count_inc, b, j);
		hcell.h_counts.inc(countIndex+cont,  count_inc);
	    }
		...
		vcell.v_counts.inc(countIndex+start,  count_inc);
		...
		vcell.p.s.update_count_encA(vcell.v_counts, count_inc, a, i);
		vcell.v_counts.inc(countIndex+cont,  count_inc);
		...
		dcell.d_counts.inc(countIndex+diag,  count_inc);
