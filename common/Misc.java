...
public final class Misc {
    ...
    static String numFmt(float num, int prec, int width, boolean pre0, boolean right) {
	return numFmt((double)num, prec, width, pre0, right);
    }

    static String numFmt(double num, int prec, int width, boolean pre0, boolean right) {
	if (prec<0)
    ...
    /** An extremely limited and poorly implemented sprintf function.
	Only handles %s %d %f  with width and precision */
	...
	while (i<fmt.length()) {
		...
		if (c=='d') {
			...
			res.append(numFmt( ((Integer)o).intValue(), width, pre0, rightAlign) );
		    } else {
		...
		if (c=='f') {
			...
			res.append(numFmt( ((Double)o).doubleValue(), prec, width, pre0, rightAlign) );
		    } else if (o instanceof Float) {
			res.append(numFmt( ((Float)o).floatValue(), prec, width, pre0, rightAlign) );
		    } else {
			System.err.println("Format %d not passed an double or float");
			res.append(o);
		    }
		    p++;
		    break;
		...
		if (c=='s') {
		    res.append( pad(objs[obj_i++].toString(), width, false, rightAlign));
		    p++;
		    break;
		}
    ...
    /** Can use like this:
	sprintf( fmt, new VarArgs(i1).add(i2).add(i3).add(i4));
    */
	...
	printf(fmt, new Object[] {new Integer(v1), new Integer(v2), new Integer(v3),});
    }

    public static void printf(String fmt, int v1, int v2, int v3, int v4) {
	printf(fmt, new Object[] {new Integer(v1), new Integer(v2), new Integer(v3), new Integer(v4)});
    }
	...
	printf(fmt, new Object[] {new Double(v1), new Double(v2), new Double(v3),});
    }

    public static void printf(String fmt, double v1, double v2, double v3, double v4) {
	printf(fmt, new Object[] {new Double(v1), new Double(v2), new Double(v3), new Double(v4)});
    }

    
    public static class VarArgs {
	Vector o = new Vector();
	public VarArgs(char v)   { add(v); }
	public VarArgs(int v)    { add(v); }
	public VarArgs(double v) { add(v); }
	public VarArgs(float v)  { add(v); }
	public VarArgs(Object v) { add(v); }

	public VarArgs add(char v)   { return add(new Character(v)); }
	public VarArgs add(int v)    { return add(new Integer(v));   }
	public VarArgs add(double v) { return add(new Double(v));    }
	public VarArgs add(float v)  { return add(new Float(v));     }
	public VarArgs add(Object v) { 
	    o.add(v);
	    return this;
	}

	public Object[] toArray() { return o.toArray(); }
    }


    public static void main(String args[]) {
	System.out.println(  sprintf(args[0], new Object[] {new Double(5), new String("abc")}));
